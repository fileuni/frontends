        ...
        {playlist.length > 1 && showList && (
          <div className="w-64 border-r border-border bg-accent/5 flex flex-col shrink-0 z-40 animate-in slide-in-from-left duration-300">
            <div className="p-4 border-b border-border flex items-center justify-between">
              <span className="text-sm font-black tracking-widest text-muted-foreground/40">{t('filemanager.previewModal.playlist')}</span>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {playlist.map((f, i) => (
                ...
                >
                  <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center shrink-0 text-[14px] font-black bg-accent", i === currentIndex ? "bg-primary text-primary-foreground" : "text-muted-foreground/40")}>
                      {f.name.split('.').pop()}
                  </div>
