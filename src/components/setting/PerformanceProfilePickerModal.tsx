...
export const PerformanceProfilePickerModal: React.FC<PerformanceProfilePickerModalProps> = ({
  isOpen,
  value,
  onClose,
  onSelect,
      ...
      <div className={cn('relative w-full max-w-xl rounded-2xl border p-4 shadow-2xl', isDark ? 'border-white/10 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-900')}>
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className={cn('inline-flex h-10 w-10 items-center justify-center rounded-xl border', isDark ? 'border-white/10 bg-white/[0.03] text-slate-100' : 'border-slate-200 bg-slate-50 text-slate-800')}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {(['low-concurrency', 'high-concurrency'] as const).map((profile) => {
            ...
            return (
              ...
              >
                <div className="text-sm font-black">{t(`admin.config.quickWizard.performance.loadProfile.${profile}`)}</div>
                <div className={cn('mt-2 text-xs leading-5', active ? 'text-primary/90' : isDark ? 'text-slate-300' : 'text-slate-500')}>
                  {profile === 'low-concurrency'
                    ? t('admin.config.quickWizard.performance.loadProfile.lowConcurrencyDesc')
                    : t('admin.config.quickWizard.performance.loadProfile.highConcurrencyDesc')}
                </div>
              </button>
            );
          })}
