...
const tierOptions: Array<{ value: PerformanceTier; labelKey: string; descKey: string }> = [
  { value: 'constrained', labelKey: 'admin.config.quickWizard.performance.tiers.constrained', descKey: 'admin.config.quickWizard.performance.descriptions.constrained' },
  { value: 'lightweight', labelKey: 'admin.config.quickWizard.performance.tiers.lightweight', descKey: 'admin.config.quickWizard.performance.descriptions.lightweight' },
  { value: 'performance', labelKey: 'admin.config.quickWizard.performance.tiers.performance', descKey: 'admin.config.quickWizard.performance.descriptions.performance' },
];

type FeatureToggleKey = 'compression' | 'sftp' | 'ftp' | 's3' | 'chat' | 'email' | 'webdav' | 'bloomWarmup';

type FeatureToggleState = Record<FeatureToggleKey, boolean>;

const featureToggleOrder: FeatureToggleKey[] = ['compression', 'sftp', 'ftp', 's3', 'chat', 'email', 'webdav', 'bloomWarmup'];
...
const readFeatureToggleStateFromConfig = (config: ConfigObject | null | undefined, fallback: FeatureToggleState): FeatureToggleState => {
  const source = (config ?? {}) as any;
  const vfs = source.vfs_storage_hub ?? {};
  const taskRegistry = source.task_registry ?? {};

  return {
    compression: typeof vfs.file_compress?.enable === 'boolean' ? vfs.file_compress.enable : fallback.compression,
    sftp: typeof vfs.enable_sftp === 'boolean' ? vfs.enable_sftp : fallback.sftp,
    ftp: typeof vfs.enable_ftp === 'boolean' ? vfs.enable_ftp : fallback.ftp,
    s3: typeof vfs.enable_s3 === 'boolean' ? vfs.enable_s3 : fallback.s3,
    chat: typeof source.chat_manager?.enabled === 'boolean' ? source.chat_manager.enabled : fallback.chat,
    email: typeof source.email_manager?.enabled === 'boolean' ? source.email_manager.enabled : fallback.email,
    webdav: typeof vfs.enable_webdav === 'boolean' ? vfs.enable_webdav : fallback.webdav,
    bloomWarmup: typeof taskRegistry.bloom_filter_warmup?.enabled === 'boolean' ? taskRegistry.bloom_filter_warmup.enabled : fallback.bloomWarmup,
  };
};
...
const applyFeatureToggleStateToConfig = (
  ...
  } else if (typeof sftpServ.max_connections !== 'number' || sftpServ.max_connections <= 1) {
    sftpServ.max_connections = draft.performanceTier === 'performance' ? 100 : 20;
    sftpServ.worker_threads = draft.performanceTier === 'performance' ? 4 : 2;
  }

  if (!toggles.ftp) {
    ftpServ.max_connections = 1;
  } else if (typeof ftpServ.max_connections !== 'number' || ftpServ.max_connections <= 1) {
    ftpServ.max_connections = draft.performanceTier === 'performance' ? 100 : 20;
  }

  if (!toggles.s3) {
    s3Serv.max_connections = 1;
  } else if (typeof s3Serv.max_connections !== 'number' || s3Serv.max_connections <= 1) {
...
const useDraft = ({ tomlAdapter, content, runtimeOs, systemHardware }: Pick<BaseProps, 'tomlAdapter' | 'content' | 'runtimeOs' | 'systemHardware'>) => {
...
export const PerformanceInlinePanel: React.FC<BaseProps> = ({ tomlAdapter, content, onContentChange, runtimeOs, systemHardware }) => {
  ...
  const { fallbackPolicy, parsed } = useDraft({ tomlAdapter, content, runtimeOs, systemHardware });

  const suggestedTemplate = parsePerformanceTemplateId(systemHardware?.suggested_performance_template);

  const currentFeatureToggles = useMemo(
    () => readFeatureToggleStateFromConfig(parsed.value, { compression: false, sftp: false, ftp: false, s3: false, chat: false, email: false, webdav: true, bloomWarmup: false }),
    [parsed.value],
  );
  ...
  const [selectedLoadProfile, setSelectedLoadProfile] = useState<'low-concurrency' | 'high-concurrency'>('low-concurrency');
  ...
  const toggleFeature = (key: FeatureToggleKey, enabled: boolean) => {
    ...
    const next = applyFeatureToggleStateToConfig(
      ...
      { performanceTier: selectedTier ?? 'lightweight', loadProfile: selectedLoadProfile } as FriendlyDraft,
    );
  ...
  const applyTemplate = () => {
    ...
    const templateId = selectedTier === 'performance'
      ? (selectedLoadProfile === 'high-concurrency' ? 'performance-high-concurrency' : 'performance-low-concurrency')
      : selectedTier;
    const templateDraft = applyPerformanceTemplateToDraft(
      parsed.value ? buildDraftFromConfig(parsed.value, fallbackPolicy) : defaultDraft,
      templateId,
    );
    templateDraft.allocatorPolicy = fallbackPolicy;
    const next = applyDraftToConfig(parsed.value, templateDraft, fallbackPolicy);
    const featureToggles = deriveFeatureToggleState(templateDraft);
    const withFeatures = applyFeatureToggleStateToConfig(next as ConfigObject, featureToggles, templateDraft);
    onContentChange(tomlAdapter.stringify(withFeatures));
    setSelectedTier(null);
  };

  type SummaryEntry = { label: string; path: string; currentValue: string; templateValue: string | null };

  const extractSummaryFromConfig = useMemo(() => (cfg: any): Array<{ label: string; path: string; value: string }> => {
    ...
    return [
      { label: t('admin.config.quickWizard.performance.preview.dbPool'), path: 'database.*.max_connections', value: `${db.sqlite_config?.max_connections ?? db.postgres_config?.max_connections ?? '-'} / ${db.sqlite_config?.min_connections ?? db.postgres_config?.min_connections ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.cacheMemory'), path: 'fast_kv_storage_hub.dashmap_mem_max_bytes', value: `${Math.round((Number(kv.dashmap_mem_max_bytes ?? 0) / 1024 / 1024) || 0)} MB` },
      { label: t('admin.config.quickWizard.performance.preview.bruteForceLockout'), path: 'middleware.brute_force.lockout_secs', value: `${bruteForce.lockout_secs ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.captchaPreheatPool'), path: 'captcha_code.graphic_cache_size', value: `${captcha.graphic_cache_size ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.captchaGenConcurrency'), path: 'captcha_code.graphic_gen_concurrency', value: `${captcha.graphic_gen_concurrency ?? '-'}/${captcha.max_gen_concurrency ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.sftpMaxConnections'), path: 'file_manager_serv_sftp.max_connections', value: `${cfg?.file_manager_serv_sftp?.max_connections ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.ftpMaxConnections'), path: 'file_manager_serv_ftp.max_connections', value: `${cfg?.file_manager_serv_ftp?.max_connections ?? '-'}` },
      { label: t('admin.config.quickWizard.performance.preview.s3MaxConnections'), path: 'file_manager_serv_s3.max_connections', value: `${cfg?.file_manager_serv_s3?.max_connections ?? '-'}` },
    ];
  }, [t]);
  ...
  const summaryEntries: SummaryEntry[] = useMemo(() => {
    ...
    if (!selectedTier || !parsed.value) {
      return currentEntries.map((e) => ({ ...e, currentValue: e.value, templateValue: null as string | null }));
    }
    const templateId = selectedTier === 'performance'
      ? (selectedLoadProfile === 'high-concurrency' ? 'performance-high-concurrency' : 'performance-low-concurrency')
      : selectedTier;
    ...
    templateDraft.allocatorPolicy = fallbackPolicy;
    const templateConfig = applyDraftToConfig(parsed.value, templateDraft, fallbackPolicy);
    const templateFeatures = deriveFeatureToggleState(templateDraft);
    const templateFull = applyFeatureToggleStateToConfig(templateConfig as ConfigObject, templateFeatures, templateDraft);
    const templateEntries = extractSummaryFromConfig(templateFull);
    return currentEntries.map((e, i) => ({
      ...e,
      currentValue: e.value,
      templateValue: templateEntries[i]?.value !== e.value ? (templateEntries[i]?.value ?? null) : null,
    }));
  }, [parsed.value, selectedTier, selectedLoadProfile, fallbackPolicy, extractSummaryFromConfig]);

  const [editingParam, setEditingParam] = useState<{ path: string; label: string; value: string } | null>(null);
  const [editingValue, setEditingValue] = useState('');
  const returnToMoreRef = React.useRef(false);

  const openParamEditor = (entry: SummaryEntry) => {
    returnToMoreRef.current = false;
    setEditingParam({ path: entry.path, label: entry.label, value: entry.currentValue });
  ...
  const openParamEditorFromMore = (item: { path: string; value: unknown }) => {
    ...
    setEditingParam({ path: item.path, label: item.path, value: String(item.value) });
  ...
  useEscapeToCloseTopLayer({ active: Boolean(editingParam), onEscape: closeParamEditor });
  useEscapeToCloseTopLayer({ active: showAllImpacts && !editingParam, onEscape: () => setShowAllImpacts(false) });
  ...
  const allConfigItems = useMemo(() => {
    ...
    return [
      ...
      { path: 'database.health_check_timeout_seconds', value: database.health_check_timeout_seconds },
      { path: 'database.sqlite_config.max_connections', value: sqlite.max_connections },
      { path: 'database.sqlite_config.max_connections_low_memory', value: sqlite.max_connections_low_memory },
      { path: 'database.sqlite_config.max_connections_throughput', value: sqlite.max_connections_throughput },
      { path: 'database.sqlite_config.min_connections', value: sqlite.min_connections },
      ...
      { path: 'database.postgres_config.max_connections', value: postgres.max_connections },
      { path: 'database.postgres_config.max_connections_low_memory', value: postgres.max_connections_low_memory },
      { path: 'database.postgres_config.max_connections_throughput', value: postgres.max_connections_throughput },
      { path: 'database.postgres_config.min_connections', value: postgres.min_connections },
      ...
      { path: 'fast_kv_storage_hub.dashmap_mem_upper_limit_ratio', value: kv.dashmap_mem_upper_limit_ratio },
      { path: 'fast_kv_storage_hub.dashmap_mem_max_bytes', value: kv.dashmap_mem_max_bytes },
      { path: 'fast_kv_storage_hub.dashmap_mem_max_bytes_low_memory', value: kv.dashmap_mem_max_bytes_low_memory },
      { path: 'fast_kv_storage_hub.dashmap_mem_max_bytes_throughput', value: kv.dashmap_mem_max_bytes_throughput },
      { path: 'internal_notify.unread_count_cache_ttl', value: cfg.internal_notify?.unread_count_cache_ttl },
      { path: 'internal_notify.retention_days', value: cfg.internal_notify?.retention_days },
      { path: 'system_backup.max_backup_size_mb', value: cfg.system_backup?.max_backup_size_mb },
      { path: 'middleware.ip_rate_limit.window_secs', value: ipRate.window_secs },
      { path: 'middleware.ip_rate_limit.max_requests', value: ipRate.max_requests },
      { path: 'middleware.client_id_rate_limit.window_secs', value: clientRate.window_secs },
      { path: 'middleware.client_id_rate_limit.max_requests', value: clientRate.max_requests },
      { path: 'middleware.client_id_rate_limit.max_cid', value: clientRate.max_cid },
      { path: 'middleware.user_id_rate_limit.window_secs', value: userRate.window_secs },
      { path: 'middleware.user_id_rate_limit.max_requests', value: userRate.max_requests },
      { path: 'middleware.user_id_rate_limit.max_userid', value: userRate.max_userid },
      ...
      { path: 'captcha_code.graphic_cache_size', value: captcha.graphic_cache_size },
      { path: 'captcha_code.graphic_gen_concurrency', value: captcha.graphic_gen_concurrency },
      { path: 'captcha_code.max_gen_concurrency', value: captcha.max_gen_concurrency },
      { path: 'middleware.brute_force.enabled', value: bruteForce.enabled },
      { path: 'middleware.brute_force.max_failures_per_user_ip', value: bruteForce.max_failures_per_user_ip },
      { path: 'middleware.brute_force.max_failures_per_ip_global', value: bruteForce.max_failures_per_ip_global },
      { path: 'middleware.brute_force.lockout_secs', value: bruteForce.lockout_secs },
      { path: 'middleware.brute_force.enable_exponential_backoff', value: bruteForce.enable_exponential_backoff },
      { path: 'memory_allocator.policy', value: cfg.memory_allocator?.policy },
      { path: 'safeaccess_guard.bloom_filter_capacity', value: cfg.safeaccess_guard?.bloom_filter_capacity },
      { path: 'file_manager_serv_sftp.max_connections', value: cfg.file_manager_serv_sftp?.max_connections },
      { path: 'file_manager_serv_ftp.max_connections', value: cfg.file_manager_serv_ftp?.max_connections },
      { path: 'file_manager_serv_s3.max_connections', value: cfg.file_manager_serv_s3?.max_connections },
  ...
  const templateConfigValues = useMemo((): Map<string, string> | null => {
    ...
    const templateConfig = applyDraftToConfig(parsed.value, templateDraft, fallbackPolicy);
    const templateFeatures = deriveFeatureToggleState(templateDraft);
    const templateFull = applyFeatureToggleStateToConfig(templateConfig as ConfigObject, templateFeatures, templateDraft) as any;
  ...
  const allConfigItemsByGroup = useMemo(() => {
    const groups = new Map<string, { label: string; items: Array<{ path: string; value: unknown }> }>();
    const otherLabel = t('admin.config.quickWizard.performance.preview.groups.other');
    allConfigItems.forEach((item) => {
      const key = item.path.split('.')[0] || 'other';
      const group = groups.get(key) ?? { label: groupLabelMap[key] || otherLabel, items: [] };
  ...
  const featureStates = featureToggleOrder.map((key) => [key, currentFeatureToggles[key]] as const);
  ...
  const getTemplateDiffForFeature = (key: FeatureToggleKey, currentEnabled: boolean): boolean | null => {
  ...
  const getTemplateDiffForPath = (path: string, currentValue: unknown): string | null => {
    if (!templateConfigValues) return null;
    const templateVal = templateConfigValues.get(path);
    if (templateVal === undefined) return null;
  ...
  return (
    <>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
        <div className={cn('rounded-2xl border p-4', isDark ? 'border-white/10 bg-black/20' : 'border-slate-200 bg-white')}>
                ...
                {tierOptions.map((tier) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className={cn('text-sm font-black', selectedTier === tier.value ? 'text-primary' : isDark ? 'text-slate-100' : 'text-slate-800')}>
                        {t(tier.labelKey)}
                      </div>
                      {suggestedTemplate && (tier.value === 'performance' ? suggestedTemplate.startsWith('performance-') : suggestedTemplate === tier.value) && (
                        <div className={cn('rounded-full px-2 py-0.5 text-[11px] font-black uppercase tracking-wide', isDark ? 'bg-cyan-500/15 text-cyan-200' : 'bg-cyan-100 text-cyan-900')}>
                          {t('admin.config.quickWizard.performance.hardwareRecommendationBadge')}
                        </div>
                      )}
                    </div>
                    <div className={cn('mt-2 text-xs leading-5', selectedTier === tier.value ? 'text-primary/90' : isDark ? 'text-slate-300' : 'text-slate-500')}>
                      {t(tier.descKey)}
                    </div>
                    {tier.value === 'performance' && selectedTier === 'performance' && (
                      <div className={cn('mt-3 flex items-center justify-between gap-2 rounded-full border px-3 py-2', isDark ? 'border-white/10 bg-white/[0.04]' : 'border-slate-200 bg-white')}>
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <span className={cn('truncate rounded-full px-2.5 py-1 text-xs font-black', isDark ? 'bg-primary/20 text-primary-foreground' : 'bg-primary/10 text-primary')}>
                            {t(`admin.config.quickWizard.performance.loadProfile.${selectedLoadProfile}`)}
                          </span>
                        </div>
              ...
              {selectedTier && (
                ...
                >
                  {t('admin.config.quickWizard.performance.applyTemplate')}
                </button>
              )}

              <div className={cn('mt-3 rounded-xl border px-3 py-3 text-sm leading-6', isDark ? 'border-white/10 bg-white/[0.03] text-slate-300' : 'border-slate-200 bg-slate-50 text-slate-600')}>
          ...
          <div className="flex h-full flex-col gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <div className={cn('text-xs font-black uppercase tracking-[0.18em]', isDark ? 'text-slate-300' : 'text-slate-700')}>
                  {t('admin.config.quickWizard.performance.featureEnablementTitle')}
                </div>
              </div>
              <div className="mt-3 grid gap-1.5 sm:grid-cols-2">
                {featureStates.map(([key, enabled]) => {
                  ...
                  return (
                    ...
                    >
                      <span>{t(`admin.config.quickWizard.performance.features.${key}`)}</span>
                      {hasDiff && (
                        <span className="flex items-center gap-1">
                          {enabled ? <Check size={14} /> : <XIcon size={14} />}
                          <span className="text-xs">-&gt;</span>
                          {templateDiff ? <Check size={14} /> : <XIcon size={14} />}
                        </span>
                      )}
                    </button>
                  );
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      entry.templateValue
                        ...
                        : (isDark ? 'border-transparent hover:bg-white/[0.04]' : 'border-transparent hover:bg-slate-100'),
                    )}
                  >
                    <span className={cn('min-w-0 truncate', isDark ? 'text-slate-400' : 'text-slate-500')}>{entry.label}</span>
                    <span className="flex shrink-0 items-center gap-1.5">
                      <span className={cn('font-mono font-black', isDark ? 'text-slate-100' : 'text-slate-900')}>{entry.currentValue}</span>
                      {entry.templateValue && (
                        <>
                          <span className={cn('text-xs', isDark ? 'text-slate-500' : 'text-slate-400')}>-&gt;</span>
                          <span className={cn('font-mono font-black', isDark ? 'text-amber-200' : 'text-amber-800')}>{entry.templateValue}</span>
                        </>
                      )}
                    </span>
                  </button>
            ...
            <button type="button" onClick={() => setShowAllImpacts(true)} className={cn('mt-auto h-10 rounded-xl border px-4 text-sm font-black transition-colors', isDark ? 'border-white/10 bg-black/20 text-slate-100 hover:bg-white/10' : 'border-slate-200 bg-white text-slate-800 hover:bg-slate-100')}>
              {t('common.more')}
            </button>
          </div>
        </div>
        ...
        <div className={cn('relative flex max-h-[92vh] w-full max-w-5xl flex-col rounded-2xl border p-4 shadow-2xl', isDark ? 'border-white/10 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-900')}>
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className={cn('mt-1 text-sm', isDark ? 'text-slate-300' : 'text-slate-500')}>{t('admin.config.quickWizard.performance.preview.totalChanges', { count: allConfigItems.length })}</div>
            </div>
          ...
          <div className="mt-4 flex-1 overflow-y-auto">
            <div className="grid gap-4 lg:grid-cols-2">
              {allConfigItemsByGroup.map((group) => (
                <section key={group.key} className={cn('rounded-xl border p-3', isDark ? 'border-white/10 bg-black/20' : 'border-slate-200 bg-slate-50')}>
                  <div className="flex items-center justify-between gap-3">
                    <div className={cn('text-[11px] font-black uppercase tracking-[0.18em]', isDark ? 'text-slate-400' : 'text-slate-500')}>{group.label}</div>
                    <div className={cn('text-xs font-black', isDark ? 'text-slate-300' : 'text-slate-600')}>{group.items.length}</div>
                  </div>
                  <div className="mt-2 grid gap-1">
                    {group.items.map((item) => {
                      const diffVal = getTemplateDiffForPath(item.path, item.value);
                      return (
                        <button
                          key={item.path}
                          type="button"
                        ...
                        >
                          <span className={cn('font-mono break-all leading-5', isDark ? 'text-slate-300' : 'text-slate-600')}>{item.path}</span>
                          <span className="flex shrink-0 items-center gap-1.5">
                            <span className={cn('font-mono font-black leading-5', isDark ? 'text-slate-100' : 'text-slate-800')}>{String(item.value)}</span>
                            {diffVal !== null && (
                              <>
                                <span className={cn('text-xs', isDark ? 'text-slate-500' : 'text-slate-400')}>-&gt;</span>
                                <span className={cn('font-mono font-black leading-5', isDark ? 'text-amber-200' : 'text-amber-800')}>{diffVal}</span>
                              </>
                            )}
                          </span>
                        </button>
    ...
    {editingParam && (
      <div className="fixed inset-0 z-[190] flex items-center justify-center p-4">
        <button type="button" aria-label="close" className="absolute inset-0 bg-black/70" onClick={closeParamEditor} />
        <div className={cn('relative w-full max-w-md rounded-2xl border p-4 shadow-2xl', isDark ? 'border-white/10 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-900')}>
          <div className="flex items-center justify-between gap-3">
            <div className={cn('text-sm font-black', isDark ? 'text-slate-100' : 'text-slate-900')}>{editingParam.label}</div>
            <button type="button" onClick={closeParamEditor} className={cn('inline-flex h-9 w-9 items-center justify-center rounded-xl border', isDark ? 'border-white/10 bg-white/[0.03] text-slate-100' : 'border-slate-200 bg-slate-50 text-slate-800')} aria-label={t('common.close')}>
              <XIcon size={16} />
            </button>
          </div>
          <div className={cn('mt-2 text-xs font-mono break-all', isDark ? 'text-slate-400' : 'text-slate-500')}>{editingParam.path}</div>
          <input
            type="text"
            value={editingValue}
            onChange={(e) => setEditingValue(e.target.value)}
            className={cn('mt-3 h-11 w-full rounded-xl border px-3 text-sm font-mono', isDark ? 'border-white/10 bg-black/30 text-white' : 'border-slate-300 bg-white text-slate-900')}
            onKeyDown={(e) => { if (e.key === 'Enter') applyParamEdit(); }}
          />
          <div className="mt-4 flex justify-end gap-2">
            <button type="button" onClick={closeParamEditor} className={cn('h-10 rounded-xl border px-4 text-sm font-black', isDark ? 'border-white/10 text-slate-200' : 'border-slate-200 text-slate-700')}>
              {t('common.cancel')}
            </button>
            <button type="button" onClick={applyParamEdit} className="h-10 rounded-xl bg-primary px-4 text-sm font-black text-white shadow-lg shadow-primary/20">
              {t('common.confirm')}
            </button>
          </div>
        </div>
