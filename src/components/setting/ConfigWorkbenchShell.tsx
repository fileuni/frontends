...
export const ConfigWorkbenchShell: React.FC<ConfigWorkbenchShellProps> = ({
  ...
  return (
    <div className={cn(
        ...
        <div className="min-w-0 flex-1">
          ...
          {configPath && (
            <div className="mt-2 flex min-w-0 items-start gap-2">
              <div className={cn('min-w-0 flex-1 rounded-xl border px-2.5 py-1.5 font-mono text-[11px] leading-5 sm:text-xs break-all', isDark ? 'border-white/10 bg-black/20 text-slate-300' : 'border-slate-200 bg-white text-slate-700')}>
                {configPath}
              </div>
              {configPathAction}
            </div>
      ...
      <div className={cn('p-2 sm:p-4 lg:p-6', bodyClassName)}>
        {children}
      </div>
    </div>
  );
