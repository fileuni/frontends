...
export const AdminPasswordPanel: React.FC<AdminPasswordPanelProps> = ({
  ...
  const [successAdminUsername, setSuccessAdminUsername] = useState<
    string | null
  >(null);
  ...
  const resolvedTitle =
    title ||
    (mode === "modal"
      ? t("launcher.reset_admin_password")
      : t("setup.admin.title"));
  const resolvedDescription =
    description ||
    (mode === "modal"
      ? t("launcher.reset_admin_password_desc")
      : t("setup.admin.subtitle"));
  const resolvedConfirmLabel = confirmLabel || t("common.confirm");
  const resolvedWarningTitle = warningTitle || t("setup.admin.finalConfirm");
  const resolvedWarningDescription =
    warningDescription || t("setup.admin.finalConfirmDesc");
  ...
  const handleConfirm = async () => {
    if (password.length < minPasswordLength) {
      setError(
        t("launcher.messages.password_too_short") ||
  ...
  const innerContent = (
    <div
      className={cn(
        ...
        mode === "panel"
          ? "bg-card border-border rounded-3xl sm:rounded-[2.5rem] shadow-2xl"
          : cn(
              ...
              isDark
                ? "bg-slate-950 border-white/10 ring-1 ring-white/5"
    ...
    >
      <div
        className={cn(
          ...
          mode === "modal" &&
            (isDark
              ? "bg-amber-500/5 border-b border-white/5"
              : "bg-amber-50/50 border-b border-gray-100"),
        )}
      >
        {mode === "modal" && onClose && (
          <button
            ...
            className={cn(
              ...
              isDark
                ...
                : "hover:bg-gray-100 text-slate-400 hover:text-slate-900",
            )}
          >
            <X size={20} />
          </button>
        ...
        <div
          className={cn(
            ...
            mode === "modal" ? "flex-col text-center" : "flex-row",
          )}
        >
          <div
            className={cn(
              ...
              mode === "modal"
                ? cn(
                    ...
                    isDark
                      ? "bg-amber-500/20 shadow-black/40"
                      : "bg-amber-100 shadow-amber-200/50",
                  )
                : "",
            )}
          >
      ...
      >
        ...
        {successAdminUsername && (
          ...
          >
            ...
            {t("setup.admin.resetSuccessWithUser", {
              username: successAdminUsername,
              defaultValue: `Admin password applied. Current admin username: ${successAdminUsername}`,
            })}
          </div>
        )}
              ...
              <input
                ...
                className={cn(
                  ...
                  isDark
                    ...
                    : "bg-gray-50 border-gray-200 text-slate-900 focus:border-amber-500",
                )}
            ...
            {showRandomGenerator && (
              <button
                ...
                className={cn(
                  ...
                  isDark
                    ? "bg-white/5 border-white/10 text-slate-400 hover:bg-amber-500 hover:text-white hover:border-amber-500"
                    : "bg-gray-50 border-gray-200 text-slate-500 hover:bg-amber-500 hover:text-white hover:border-amber-500",
                )}
                title={
                  t("setup.admin.generate_password") ||
