...
const normalizeOptionsForDriver = (
  driver: VfsDriver,
  pairs: KvPair[],
): KvPair[] => {
  if (driver !== "s3") {
  ...
  const hasSecretAccessKey = pairs.some(
    (pair) => pair.key === "secret_access_key",
  );
  ...
  if (!hasAccessKeyId && hasLegacyAccessKey) {
    next = next.map((pair) =>
      pair.key === "access_key" ? { ...pair, key: "access_key_id" } : pair,
    );
  }
  if (!hasSecretAccessKey && hasLegacySecretKey) {
    next = next.map((pair) =>
      pair.key === "secret_key" ? { ...pair, key: "secret_access_key" } : pair,
    );
  }

  // Drop legacy keys when normalized counterparts exist.
  const normalizedHasAccessKeyId = next.some(
    (pair) => pair.key === "access_key_id",
  );
  const normalizedHasSecretAccessKey = next.some(
    (pair) => pair.key === "secret_access_key",
  );
...
const remoteConnectorFields: Record<
  RemoteConnectorDriver,
...
const isRemoteConnectorDriver = (
  driver: VfsDriver,
): driver is RemoteConnectorDriver => {
...
const parseVfsDraftFromContent = (
  ...
  try {
    ...
    const connectors: ConnectorDraft[] = Array.isArray(connectorsRaw)
      ? connectorsRaw.filter(isRecord).map((conn) => {
          const driver: VfsDriver = (
            typeof conn.driver === "string" ? conn.driver : "fs"
          ) as VfsDriver;
          const rawRoot = typeof conn.root === "string" ? conn.root : "";
          const root =
            driverUsesSlashRoot(driver) && rawRoot.trim().length === 0
              ? "/"
              : rawRoot;
          return {
            id: makeId("connector"),
    ...
    const pools: PoolDraft[] = Array.isArray(poolsRaw)
      ? poolsRaw.filter(isRecord).map((pool) => ({
          id: makeId("pool"),
          name: typeof pool.name === "string" ? pool.name : "",
          primary_connector:
            typeof pool.primary_connector === "string"
              ? pool.primary_connector
              : "",
          backup_connector:
            typeof pool.backup_connector === "string"
              ? pool.backup_connector
              : "",
          enable_write_cache:
            typeof pool.enable_write_cache === "boolean"
    ...
    const policies: PolicyDraft[] = Array.isArray(policiesRaw)
      ? policiesRaw.filter(isRecord).map((policy) => ({
          id: makeId("policy"),
          role_id: typeof policy.role_id === "string" ? policy.role_id : "",
          pool_name:
            typeof policy.pool_name === "string" ? policy.pool_name : "",
          default_quota:
            typeof policy.default_quota === "number"
              ? String(policy.default_quota)
              : "",
          max_private_mounts:
            typeof policy.max_private_mounts === "number"
              ? String(policy.max_private_mounts)
              : "",
          min_mount_sync_interval_minutes:
            typeof policy.min_mount_sync_interval_minutes === "number"
              ? String(policy.min_mount_sync_interval_minutes)
              : "",
          max_mount_sync_timeout_secs:
            typeof policy.max_mount_sync_timeout_secs === "number"
              ? String(policy.max_mount_sync_timeout_secs)
              : "",
        }))
      : [];

    const effectiveConnectors =
      connectors.length > 0 ? connectors : defaults.connectors;
    const effectivePools = pools.length > 0 ? pools : defaults.pools;
    const poolNames = new Set(
      effectivePools.map((p) => p.name).filter((v) => v.trim().length > 0),
    );
    const resolvedDefaultPool =
      typeof defaultPoolRaw === "string" && defaultPoolRaw.trim().length > 0
        ? defaultPoolRaw
...
const validateDraft = (
  ...
  if (connectors.length === 0) {
    errors.push(
      t("admin.config.storage.validation.errors.atLeastOneConnector"),
    );
  }
  ...
  if (connectorNames.length !== connectorNameSet.size) {
    errors.push(
      t("admin.config.storage.validation.errors.connectorNamesUnique"),
    );
  }
  ...
  connectors.forEach((c, index) => {
    if (!c.name.trim())
      errors.push(
        t("admin.config.storage.validation.errors.connectorNameRequired", {
          index,
        }),
      );
    if (!c.driver.trim())
      errors.push(
        t("admin.config.storage.validation.errors.connectorDriverRequired", {
          index,
        }),
      );
    if (!c.root.trim())
      errors.push(
        t("admin.config.storage.validation.errors.connectorRootRequired", {
          index,
        }),
      );
  });
  ...
  pools.forEach((p, index) => {
    if (!p.name.trim())
      errors.push(
        t("admin.config.storage.validation.errors.poolNameRequired", { index }),
      );
    if (!p.primary_connector.trim())
      errors.push(
        t("admin.config.storage.validation.errors.poolPrimaryRequired", {
          index,
        }),
      );
    if (
      p.primary_connector.trim() &&
      !connectorNameSet.has(p.primary_connector.trim())
    ) {
      errors.push(
        t("admin.config.storage.validation.errors.poolPrimaryUnknown", {
          index,
          name: p.primary_connector,
        }),
      );
    }
    if (
      p.backup_connector.trim() &&
      !connectorNameSet.has(p.backup_connector.trim())
    ) {
      errors.push(
        t("admin.config.storage.validation.errors.poolBackupUnknown", {
          index,
          name: p.backup_connector,
        }),
      );
    }
  });
  ...
  if (!defaultPool.trim()) {
    errors.push(
      t("admin.config.storage.validation.errors.defaultPoolRequired"),
    );
  } else if (!poolNameSet.has(defaultPool.trim())) {
    errors.push(
      t("admin.config.storage.validation.errors.defaultPoolNotFound", {
        name: defaultPool,
      }),
    );
  }
  ...
  policies.forEach((policy, index) => {
    if (!policy.role_id.trim())
      errors.push(
        t("admin.config.storage.validation.errors.policyRoleRequired", {
          index,
        }),
      );
    if (!policy.pool_name.trim())
      errors.push(
        t("admin.config.storage.validation.errors.policyPoolRequired", {
          index,
        }),
      );
    if (policy.pool_name.trim() && !poolNameSet.has(policy.pool_name.trim())) {
      errors.push(
        t("admin.config.storage.validation.errors.policyPoolUnknown", {
          index,
    ...
    if (!Number.isFinite(quota) || quota < 0) {
      errors.push(
        t("admin.config.storage.validation.errors.policyQuotaInvalid", {
          index,
        }),
      );
    }
    const maxPrivateMounts = Number.parseInt(
      policy.max_private_mounts.trim(),
      10,
    );
    if (!Number.isFinite(maxPrivateMounts) || maxPrivateMounts < 0) {
      errors.push(
        t(
          "admin.config.storage.validation.errors.policyMaxPrivateMountsInvalid",
          { index },
        ),
      );
    }
    const minSyncInterval = Number.parseInt(
    ...
    if (!Number.isFinite(minSyncInterval) || minSyncInterval <= 0) {
      errors.push(
        t(
          "admin.config.storage.validation.errors.policyMinMountSyncIntervalInvalid",
          { index },
        ),
      );
    }
    const maxSyncTimeout = Number.parseInt(
    ...
    if (!Number.isFinite(maxSyncTimeout) || maxSyncTimeout <= 0) {
      errors.push(
        t(
          "admin.config.storage.validation.errors.policyMaxMountSyncTimeoutInvalid",
          { index },
        ),
      );
    }
