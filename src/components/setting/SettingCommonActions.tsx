...
import { AdminPasswordInlinePanel, CacheAccelerationInlinePanel, CompressionInlinePanel, ThumbnailInlinePanel } from './SettingInlineExternalPanels';
...
const LicenseInlinePanel: React.FC<LicenseInlinePanelProps> = ({
  ...
  const statusTone = !licenseStatus
    ? (isDark ? 'border-white/10 bg-black/20 text-slate-200' : 'border-slate-200 bg-white text-slate-700')
    : licenseStatus.is_valid
  ...
  const baseCardClassName = cn(
    ...
    isDark ? 'border-white/10 bg-black/20 text-slate-100' : 'border-slate-200 bg-white text-slate-900',
  );

  return (
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_17rem]">
      <label className={cn('block text-sm font-black', isDark ? 'text-slate-200' : 'text-slate-700')}>
        <div className="flex flex-wrap items-center gap-2">
          <span>{t('admin.config.license.title')}</span>
          <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-black uppercase tracking-[0.18em]', statusTone)}>
            {statusText}
          </span>
        </div>
        <textarea
          ...
          className={cn(
            ...
            isDark ? 'border-white/10 bg-black/30 text-slate-100 placeholder:text-slate-500' : 'border-slate-200 bg-white text-slate-900 placeholder:text-slate-400',
          )}
        />
        <div className={cn('mt-2 text-xs leading-6', isDark ? 'text-slate-400' : 'text-slate-500')}>
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
        <div className={baseCardClassName}>
          <div className="flex items-center gap-2">
            <ShieldCheck size={16} className={!licenseStatus ? (isDark ? 'text-slate-400' : 'text-slate-500') : licenseStatus.is_valid ? 'text-emerald-500' : 'text-amber-500'} />
          ...
          <div className={cn('mt-2 text-sm font-black', !licenseStatus ? (isDark ? 'text-slate-200' : 'text-slate-700') : licenseStatus.is_valid ? (isDark ? 'text-emerald-300' : 'text-emerald-700') : (isDark ? 'text-amber-300' : 'text-amber-700'))}>
            {statusText}
          </div>
        </div>
        ...
        <div className={baseCardClassName}>
          <div className="flex items-center gap-2">
            <Users size={16} className={isDark ? 'text-cyan-300' : 'text-cyan-600'} />
            <div className={cn('text-[11px] font-black uppercase tracking-[0.18em]', isDark ? 'text-slate-400' : 'text-slate-500')}>
              {t('admin.config.quickWizard.fields.maxUsers')}
            </div>
          </div>
          <div className={cn('mt-2 text-sm font-black font-mono', isDark ? 'text-slate-100' : 'text-slate-800')}>
            {licenseStatus ? `${licenseStatus.current_users} / ${licenseStatus.max_users}` : '-'}
          </div>
        </div>

        <div className={baseCardClassName}>
          <div className="flex items-center gap-2">
            <Clock3 size={16} className={isDark ? 'text-violet-300' : 'text-violet-600'} />
            <div className={cn('text-[11px] font-black uppercase tracking-[0.18em]', isDark ? 'text-slate-400' : 'text-slate-500')}>
        ...
        {licenseStatus?.msg && (
          <div className={cn('rounded-2xl border px-3 py-3 text-sm leading-6 sm:col-span-2 lg:col-span-1', isDark ? 'border-white/10 bg-white/[0.03] text-slate-300' : 'border-slate-200 bg-slate-50 text-slate-600')}>
            {licenseStatus.msg}
          </div>
        )}
      </div>
  ...
  return [
      ...
      renderPanel: (
        <div className="grid gap-4">
          <section className="space-y-3">
            ...
            <DatabaseInlinePanel tomlAdapter={tomlAdapter} content={content} onContentChange={onContentChange} runtimeOs={runtimeOs} onTestDatabase={onTestDatabase} />
          </section>
          <section className="space-y-3">
            <h4 className={cn('text-base font-black', isDark ? 'text-slate-100' : 'text-slate-900')}>{t('setup.config.kvType')}</h4>
            <CacheInlinePanel tomlAdapter={tomlAdapter} content={content} onContentChange={onContentChange} runtimeOs={runtimeOs} onTestCache={onTestCache} />
          </section>
        </div>
      ),
    },
    {
      ...
      actions: [
        ...
        { id: 'jump', label: t('admin.config.storage.title'), onClick: () => document.getElementById('setup-inline-storage')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) },
      ],
    },
    {
      id: 'admin-password',
    ...
    },
    {
      ...
      actions: [
        { id: 'save', label: t('setup.guide.card3Action'), onClick: license.onApplyLicense, variant: 'primary', disabled: license.saving },
      ],
    },
    {
