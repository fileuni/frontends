  ...
  return (
    <div className={cn("w-52 border-r border-border/40 flex flex-col bg-muted/5", activeView !== "folders" && "hidden md:flex")}>
      ...
      <div className="p-2 space-y-1 overflow-y-auto flex-1 custom-scrollbar">
        {folders.map((folder) => (
          <button type="button" key={folder.id} onClick={() => onSelectFolder(folder.id)} className={cn("w-full p-2 rounded-xl text-left transition-all flex items-center justify-between text-sm font-medium", selectedFolder === folder.id ? "bg-background shadow-md border border-border/40 text-primary text-foreground" : "hover:bg-background/40 text-muted-foreground")}>
