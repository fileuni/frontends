        ...
        <div className={glassSectionCardBase}>
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              {result.last_run_at && (
                <div className="mt-1 text-[14px] font-bold opacity-50">
          ...
          {result.last_error && (
            <div className="mt-4 rounded-2xl border border-red-500/20 bg-red-500/5 px-4 py-3">
              ...
              <div className="mt-1 break-words text-[14px] font-bold text-red-400">{result.last_error}</div>
            </div>
          )}
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              {result.detect_error_v4 && (
                <div className="mt-2 break-words text-[14px] font-bold text-red-400">{result.detect_error_v4}</div>
              )}
              {result.dns_error_v4 && (
                <div className="mt-2 break-words text-[14px] font-bold text-orange-400">{result.dns_error_v4}</div>
              )}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              {result.detect_error_v6 && (
                <div className="mt-2 break-words text-[14px] font-bold text-red-400">{result.detect_error_v6}</div>
              )}
              {result.dns_error_v6 && (
                <div className="mt-2 break-words text-[14px] font-bold text-orange-400">{result.dns_error_v6}</div>
              )}
              <div className="mt-3 flex items-center gap-2 text-[14px] font-bold">
                <Badge
                  variant="outline"
