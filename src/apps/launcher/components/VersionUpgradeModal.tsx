...
export interface VersionUpgradeStatusView {
  state: 'ready' | 'config_missing' | 'upgrade_required' | 'downgrade_blocked' | 'manual_intervention_required';
...
export interface VersionUpgradePlanStepView {
  ...
  to_version: string;
  label: string;
  kind: 'bootstrap_legacy_runtime' | 'consecutive_release' | 'major_upgrade_bridge';
}
...
const valueOrFallback = (value: string | null | undefined, fallback: string): string => {
...
export const VersionUpgradeModal: React.FC<VersionUpgradeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[170] bg-black/72 flex items-center justify-center p-2 sm:p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-xl rounded-3xl border border-amber-300/40 bg-white/95 dark:bg-slate-900/95 shadow-2xl overflow-hidden flex flex-col min-h-0 max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)]">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-6 py-5 space-y-5">
          ...
          <div className="rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 dark:border-slate-700/60 dark:bg-slate-950/40 space-y-3">
            <div className="flex items-start justify-between gap-3 text-sm">
              <span className="font-black text-slate-500 dark:text-slate-400">{t('launcher.upgrade.binaryVersion')}</span>
              <span className="font-mono text-right text-slate-800 dark:text-slate-100 break-all">{valueOrFallback(status.program_version, t('launcher.upgrade.missingValue'))}</span>
            </div>
            <div className="flex items-start justify-between gap-3 text-sm">
              <span className="font-black text-slate-500 dark:text-slate-400">{t('launcher.upgrade.environmentVersion')}</span>
              <span className="font-mono text-right text-slate-800 dark:text-slate-100 break-all">{valueOrFallback(status.config_version, t('launcher.upgrade.missingValue'))}</span>
            </div>
            <div className="flex items-start justify-between gap-3 text-sm">
              <span className="font-black text-slate-500 dark:text-slate-400">{t('launcher.upgrade.targetVersion')}</span>
              <span className="font-mono text-right text-slate-800 dark:text-slate-100 break-all">{valueOrFallback(status.target_version, t('launcher.upgrade.missingValue'))}</span>
            </div>
            <div className="flex items-start justify-between gap-3 text-sm">
              <span className="font-black text-slate-500 dark:text-slate-400">{t('launcher.upgrade.schemaVersion')}</span>
              <span className="font-mono text-right text-slate-800 dark:text-slate-100 break-all">{valueOrFallback(status.schema_version, t('launcher.upgrade.missingValue'))}</span>
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-4 dark:border-slate-700/60 dark:bg-slate-950/40 space-y-3">
              ...
              <div className="space-y-3">
                {status.planned_steps.map((step) => (
                  <div key={`${step.from_version}-${step.to_version}-${step.label}`} className="rounded-2xl border border-slate-200/70 bg-slate-50/80 px-3 py-3 dark:border-slate-800 dark:bg-slate-900/70">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-100">{step.label}</p>
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{stepKindLabel(step.kind)}</p>
                      </div>
        ...
        <div className="px-6 py-5 border-t border-slate-200/70 dark:border-slate-700/60 flex items-center justify-end gap-3 bg-slate-50/80 dark:bg-slate-950/40 shrink-0">
          ...
          {status.can_auto_upgrade && status.requires_upgrade && (
            ...
            >
              {busy ? t('launcher.upgrade.upgrading') : t('launcher.upgrade.confirm')}
            </button>
          )}
        </div>
      </div>
